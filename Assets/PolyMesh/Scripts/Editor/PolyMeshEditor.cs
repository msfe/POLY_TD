...
public class PolyMeshEditor : Editor
{
	...
	public override void OnInspectorGUI()
	{
        ...
        //Create 2D collider
        if (colliderSettings2D = EditorGUILayout.Foldout(colliderSettings2D, "Collider2D"))
        {
            if (polyMesh.polyCollider2D == null)
            {
                if (GUILayout.Button("Create 2D Collider"))
                {
                    RecordDeepUndo();

                    var obj = new GameObject("Collider2D", typeof(PolygonCollider2D));
                    obj.transform.parent = polyMesh.transform;
                    obj.transform.localPosition = Vector3.zero;
                    polyMesh.polyCollider2D = obj.GetComponent<PolygonCollider2D>();
                }
            }
            else
            {
                if (GUILayout.Button("Destroy 2D Collider"))
                {
                    RecordDeepUndo();

                    DestroyImmediate(polyMesh.polyCollider2D.gameObject);
                    polyMesh.polyCollider2D = null;
                }     
            }
        }

        //Create outline
        if (outlineSettings = EditorGUILayout.Foldout(outlineSettings, "Outline"))
        {
            if (polyMesh.outline == null)
            {
                if (GUILayout.Button("Create Outline"))
                {
                    RecordDeepUndo();

                    var obj = new GameObject("Outline", typeof(MeshFilter), typeof(MeshRenderer));
                    obj.transform.parent = polyMesh.transform;
                    obj.transform.localPosition = Vector3.zero;
                    polyMesh.outline = obj.GetComponent<MeshFilter>();
                }
            }
            else
            {
                var outlineWidth = EditorGUILayout.FloatField("Outline Width", polyMesh.outlineWidth);
                
                var outlineGlobalUv = EditorGUILayout.Toggle("Global UV", polyMesh.outlineGlobalUv);

                
                if (outlineGlobalUv)
                {
                    var outlineUvPosition = EditorGUILayout.Vector2Field("Position", polyMesh.outlineUvPosition);
                    var outlineUvScale = EditorGUILayout.FloatField("Scale", polyMesh.outlineUvScale);
                    var outlineUvRotation = EditorGUILayout.Slider("Rotation", polyMesh.outlineUvRotation, -180, 180) % 360;

                    if (outlineUvRotation < -180)
                        outlineUvRotation += 360;

                    if (GUI.changed)
                    {
                        RecordUndo();
                        polyMesh.outlineGlobalUv = outlineGlobalUv;
                        polyMesh.outlineUvPosition = outlineUvPosition;
                        polyMesh.outlineUvScale = outlineUvScale;
                        polyMesh.outlineUvRotation = outlineUvRotation;
                    }

                    if (GUILayout.Button("Reset UVs"))
                    {
                        polyMesh.outlineUvPosition = Vector3.zero;
                        polyMesh.outlineUvScale = 1;
                        polyMesh.outlineUvRotation = 0;
                    }
                }
                else
                {
                    var outlineUvOffset = EditorGUILayout.FloatField("Offset", polyMesh.outlineUvOffset);
                    var outlineUvScale = EditorGUILayout.FloatField("Scale", polyMesh.outlineUvScale);
                    ...
                    if (GUI.changed)
                    {
                        RecordUndo();
                        polyMesh.outlineGlobalUv = outlineGlobalUv;
                        polyMesh.outlineUvOffset = outlineUvOffset;
                        polyMesh.outlineUvScale = outlineUvScale;
                    }
                }
                ...
                {
                    RecordUndo();
                    polyMesh.outlineWidth = outlineWidth;
                }

                if (GUILayout.Button("Destroy Outline"))
                {
                    RecordDeepUndo();

                    DestroyImmediate(polyMesh.outline.gameObject);
                    polyMesh.outline = null;
                }
            }
        }
    ...
    static bool colliderSettings2D
    {
        get { return EditorPrefs.GetBool("PolyMeshEditor_colliderSettings2D", false); }
        set { EditorPrefs.SetBool("PolyMeshEditor_colliderSettings2D", value); }
    }

    static bool outlineSettings
    {
        get { return EditorPrefs.GetBool("PolyMeshEditor_outlinerSettings", false); }
        set { EditorPrefs.SetBool("PolyMeshEditor_outlinerSettings", value); }
    }
